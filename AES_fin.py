path = '/Users/navodaytomar/Desktop/209/HW1/plaintext.txt'
path1 = '/Users/navodaytomar/Desktop/209/HW1/encryptedtext.txt'
path2 = '/Users/navodaytomar/Desktop/209/HW1/decryptedtext.txt'

sbox = [
    
]

sboxInv = [
    
]

rcon = [
    
]

# returns a copy of the word shifted n bytes (chars)
# positive values for n shift bytes left, negative values shift right
#returns the words after shift, positive for bytes after shifting left and negative values for shifting right
def rotate(word, n):
    return word[n:] + word[0:n]

#sbox transform for each value of the state/message
def SubBytes(state):
    for i in range(len(state)):
        state[i] = sbox[state[i]]

# inverse sbox transform on each byte in state table
def subBytesInv(state):
    for i in range(len(state)):
        state[i] = sboxInv[state[i]]

#iterate over each row and shift bytes to the left appropriately
def ShiftRows(state):
    for i in range(4):
        state[i * 4:i * 4 + 4] = rotate(state[i * 4:i * 4 + 4], i)

#iterate over each row and shift bytes to the right appropriately
def shiftRowsInv(state):
    for i in range(4):
        state[i * 4:i * 4 + 4] = rotate(state[i * 4:i * 4 + 4], -i)

# Galois Multiplication
def galoisMult(a, b):
    p = 0
    hiBitSet = 0
    for i in range(8):
        if b & 1 == 1:
            p ^= a
        hiBitSet = a & 0x80
        a <<= 1
        if hiBitSet == 0x80:
            a ^= 0x1b
        b >>= 1
    return p % 256

#takes a column and operates on it
def MixColumn(column):
    temp = copy(column)
    column[0] = galoisMult(temp[0], 2) ^ galoisMult(temp[3], 1) ^ \
                galoisMult(temp[2], 1) ^ galoisMult(temp[1], 3)
    column[1] = galoisMult(temp[1], 2) ^ galoisMult(temp[0], 1) ^ \
                galoisMult(temp[3], 1) ^ galoisMult(temp[2], 3)
    column[2] = galoisMult(temp[2], 2) ^ galoisMult(temp[1], 1) ^ \
                galoisMult(temp[0], 1) ^ galoisMult(temp[3], 3)
    column[3] = galoisMult(temp[3], 2) ^ galoisMult(temp[2], 1) ^ \
                galoisMult(temp[1], 1) ^ galoisMult(temp[0], 3)

#takes a column and operates on it
def mixColumnInv(column):
    temp = copy(column)
    column[0] = galoisMult(temp[0], 14) ^ galoisMult(temp[3], 9) ^ \
                galoisMult(temp[2], 13) ^ galoisMult(temp[1], 11)
    column[1] = galoisMult(temp[1], 14) ^ galoisMult(temp[0], 9) ^ \
                galoisMult(temp[3], 13) ^ galoisMult(temp[2], 11)
    column[2] = galoisMult(temp[2], 14) ^ galoisMult(temp[1], 9) ^ \
                galoisMult(temp[0], 13) ^ galoisMult(temp[3], 11)
    column[3] = galoisMult(temp[3], 14) ^ galoisMult(temp[2], 9) ^ \
                galoisMult(temp[1], 13) ^ galoisMult(temp[0], 11)

#generates a column from the state table and does Galois multiplication on it
def MixColumns(state):
    for i in range(4):
        column = []
        for j in range(4):
            column.append(state[j * 4 + i])
        MixColumn(column)
        state[j * 4 + i] = column[j]

#each byte of the state/message is XORed with the key
def AddRoundKey(state,RoundKey):
    for i in range(len(state)):
        state[i] = state[i] ^ RoundKey[i]

def KeyExpansion():


def encrypt(message,key):
    state = message
    num_of_rounds = 1

    KeyExpansion()
    AddRoundKey(state,key) #Add round key
    for i in range(num_of_rounds):
        SubBytes(state)
        ShiftRows(state)
        MixColumns(state)
        AddRoundKey(state,key)
    #Final Round
    SubBytes(state)
    ShiftRows(state)
    AddRoundKey(state,key)

file_rd = open(path,'rw')
message = file_rd.read()
print message
key = 'lukeimyourfather'
state = message[:16]
print state
print key
encrypt(message,key)